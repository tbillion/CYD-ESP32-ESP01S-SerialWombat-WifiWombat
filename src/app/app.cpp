...
void App::initHardware() {
  ...
  msg_info("i2c", I2C_INIT_BEGIN, "I2C Initialization", "Configuring I2C bus: SDA=%d, SCL=%d",
           g_cfg.i2c_sda, g_cfg.i2c_scl);

  Wire.begin(g_cfg.i2c_sda, g_cfg.i2c_scl);
  Wire.setClock(100000);

  msg_info("i2c", I2C_BUS_OK, "I2C Bus Ready", "I2C initialized at 100kHz");
...
void App::initSD() {
  ...
  if (SD_SUPPORT_ENABLED && g_cfg.sd_enable) {
    if (sdMount()) {
      ...
      msg_info("sd", SD_MOUNT_OK, "SD Card Ready", "SD card detected and mounted");
    } else {
...
void App::initNetwork() {
  ...
  if (!wm.autoConnect("Wombat-Setup")) {
    ...
    boot_stage_ok(BootStage::BOOT_07_NETWORK, "WiFi connected: %s (IP: %s)", WiFi.SSID().c_str(),
  ...
  if (WiFi.getMode() == WIFI_MODE_STA && WiFi.status() == WL_CONNECTED) {
    ...
    while (retry < maxRetries) {
      if (getLocalTime(&timeinfo, 500)) {
        ...
        msg_info("time", BOOT_08_TIME_OK, "Time Synchronized", "System time set via NTP: %s UTC",
                 timeStr);
        return;
      }
      retry++;
...
void App::initWebServer() {
  ...
  server.on(
      "/upload_hex", HTTP_POST, []() { handleUploadHexPost(App::getInstance().getWebServer()); },
  ...
  // ===================================================================================
  server.on("/api/health", HTTP_GET, []() { handleApiHealth(App::getInstance().getWebServer()); });

  // ===================================================================================
  // System Settings API
  // ===================================================================================
  server.on("/api/system", HTTP_GET, []() { handleApiSystem(App::getInstance().getWebServer()); });
  ...
  server.on("/api/apply", HTTP_POST, []() { handleApiApply(App::getInstance().getWebServer()); });
  ...
  tcpServer.begin();
  msg_info("tcp", TCP_BRIDGE_START, "TCP Bridge Started", "TCP bridge listening on port %d",
           TCP_PORT);

  boot_stage_ok(BootStage::BOOT_09_SERVICES,
                "Web server (port 80) and TCP bridge (port %d) started", TCP_PORT);
}
...
void App::initOTA() {
  ...
  ArduinoOTA.onEnd([]() { msg_info("ota", OTA_UPDATE_OK, "OTA Update Complete", "Rebooting..."); });
  ...
  ArduinoOTA.onError([](ota_error_t error) {
    ...
    if (error == OTA_AUTH_ERROR)
      ...
      err_msg = "Begin Failed";
    else if (error == OTA_CONNECT_ERROR)
      err_msg = "Connect Failed";
    else if (error == OTA_RECEIVE_ERROR)
      err_msg = "Receive Failed";
