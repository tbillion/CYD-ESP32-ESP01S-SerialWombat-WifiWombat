...
struct MessageSummary {
  ...
  MessageSummary()
      : active_count(0),
        history_count(0),
        highest_active_severity(MessageSeverity::INFO),
        sequence(0) {}
};
...
class MessageCenter {
 public:
  ...
  // Returns: message ID
  uint32_t post(MessageSeverity severity, const char* source, const char* code, const char* title,
                const char* details = "");

  // Post with formatted details (printf-style)
  uint32_t postf(MessageSeverity severity, const char* source, const char* code, const char* title,
                 const char* fmt, ...);
