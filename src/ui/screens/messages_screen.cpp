...
static void populateActiveList() {
  ...
  for (const auto& msg : messages) {
    ...
    snprintf(buf, sizeof(buf), "%s [%02lu:%02lu:%02lu] %s", severityToIcon(msg.severity),
             (sec / 3600), ((sec / 60) % 60), (sec % 60), msg.title.c_str());
...
void showMessageDetail(uint32_t msg_id) {
  ...
  snprintf(buf, sizeof(buf),
           ...
           "Title: %s\n"
           "Details: %s",
           severityToIcon(msg->severity), severityToStr(msg->severity), msg->source.c_str(),
           msg->code.c_str(), (sec / 3600), ((sec / 60) % 60), (sec % 60), msg->count,
           msg->title.c_str(), msg->details.c_str());
