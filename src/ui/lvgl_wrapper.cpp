...
#  if defined(BOARD_HAS_PSRAM)
  g_lv_buf1 = (lv_color_t*)heap_caps_malloc(buf_px * sizeof(lv_color_t),
                                            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  g_lv_buf2 = (lv_color_t*)heap_caps_malloc(buf_px * sizeof(lv_color_t),
                                            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
#  endif
...
#  if LVGL_VERSION_MAJOR >= 9
  ...
  lv_display_set_buffers(g_lv_disp, g_lv_buf1, g_lv_buf2, buf_px * sizeof(lv_color_t),
...
void lvglTickAndUpdate() {
  ...
  if (g_lbl_i2c) {
    char buf[32];
    snprintf(buf, sizeof(buf), "I2C: %lu/%lu", (unsigned long)g_i2c_tx_count,
             (unsigned long)g_i2c_rx_count);
